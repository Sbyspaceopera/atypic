...
export default function Edit({attributes, setAttributes}) {
	...
	useEffect(()=>{
		apiFetch({path:'/atypic/v1/logo'}).then((data)=>setAttributes({logoUrl:data}));
	}, [])
	useEffect(()=>{
		apiFetch({path:'/'}).then((data)=>setAttributes({siteTitle:data.name}));
	}, [])
	useEffect(()=>{
		apiFetch(
				{
			...
			.then((data)=>setAttributes({siteTitle:data.name})).catch(setAttributes({menu:{error:"Please setup the Main Menu"}}));
	}, [])
		...
		<div {...useBlockProps()}>
			<header className="min-h-[15vh] z-10 top-0 border-solid border-t-0 border-r-0 border-l-0 border-b-4 atypic-header bg-white w-full p-3 sm:p-5 flex items-center justify-between">
				<div className="flex gap-1 flex-wrap justify-center sm:gap-3">
					...
					<a className="justify-end self-end text-left text-4xl sm:text-6xl text-black underline decoration-dotted decoration-yellow-500" href="<?php echo home_url() ?>">
